...
// The format the the drive will be initalized to. When inserting disks of different format, there will be a delay while
// the code detects the disk format
...
// Defect the floppy disc format between each read. Will be faster when switching between formats, but slower is using
// all the same (detecting a switch dymically is about 2.5 extra seconds verse 400ms each time)
#ifndef RE_DETECT
#define RE_DETECT 0
#endif